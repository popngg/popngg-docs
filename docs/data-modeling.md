...
기존 `battle_credit`, `local_credit`은 신규 credit 체계로 그대로 가져가지 않습니다. 실제 원천 의미를 확인한 뒤 매핑하거나 0으로 초기화합니다.
...
이번 버전부터 게임은 기존 점수를 초기화하고, 이번 버전 베스트 점수와 역대 베스트 점수를 나눠서 가집니다. 따라서 `playdata`는 유저별 chart당 하나가 아니라, 베스트 스코프별로 하나씩 저장합니다.
...
Upper를 같은 song의 chart 속성으로 둘 경우에는 `is_upper`와 `chart_version`을 songhash seed에 넣지 않는 방향을 우선 검토합니다. 이 경우 같은 곡의 일반 채보와 Upper 채보는 같은 `song_hash`를 공유하고, 실제 채보 식별은 `chart_id` 또는 `song_hash + difficulty + is_upper`로 합니다.
