...
!!! info "확인 기준"
    [`popngg/popngg`](https://github.com/popngg/popngg) 저장소의 `9a50345` 커밋(2026-08-30)을 기준으로 확인했습니다. 세부 계약은 항상 현재 코드와 Swagger UI를 우선합니다.
...
Prometheus와 Grafana는 `deploy/compose.monitoring.yml`로 선택 실행합니다. 공개 health check는 `/health`이며, 전체 Actuator와 `/actuator/prometheus`는 컨테이너 내부 관리 포트 `9091`에서만 제공합니다. Grafana만 HTTPS reverse proxy를 통해 `grafana.popn.gg`로 공개하고 Prometheus는 외부에 직접 공개하지 않습니다.
...
- Prometheus/Grafana 이후 Loki·Alloy·Alertmanager의 실제 배포 시점과 alert 기준
