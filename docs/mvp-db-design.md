...
기존 DB의 `battle_credit`, `local_credit`은 신규 credit 체계와 1:1 매핑하지 않습니다. 마이그레이션 전 실제 의미를 확인하고, 매핑할 수 없으면 0으로 초기화하거나 별도 리포트에 남깁니다.
...
```text
song_hash = md5(normalizedGenreName + "|" + normalizedSongName + "|" + normalizedArtistName + "|" + songVersion + "|" + upperGroupPolicy)
```
...
- 기본 방향은 Upper를 별도 song으로 나누지 않고 같은 song의 chart 속성으로 둡니다. 같은 곡이라도 Upper가 나온 버전은 다를 수 있으므로, Upper 출시 버전은 `songs.version`이 아니라 `charts.chart_version`에 저장합니다.
...
High☆Cheers부터 기존 점수가 초기화되고, 게임은 이번 버전 베스트 점수와 역대 베스트 점수를 나눠서 가집니다. 따라서 `playdata`는 단순히 유저별 최신 점수 하나를 저장하지 않습니다. 어떤 스코프의 베스트인지와 그 점수가 실제로 나온 게임 버전을 함께 저장합니다.
...
프론트에서 현재 버전 베스트와 역대 베스트가 모두 필요하면 API가 두 스코프를 명시적으로 조회하거나, 응답을 스코프별로 분리해서 내려줍니다. 스코프 없는 전체 조회를 기본값으로 두지 않습니다.
...
| 기존 | 신규 | 변환 |
| --- | --- | --- |
| `"user"` | `users` | `password` -> `password_hash`, `"character"` -> `character_name`, 기존 `popclass` -> `legacy_popclass`, `potential_popclass`는 `ALL_TIME_BEST` 적재 후 재계산, credit은 High☆Cheers 4종으로 재매핑, `email`은 신규 nullable |
